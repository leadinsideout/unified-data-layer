...
- ✅ Working Express API with transcript upload, search, and RAG endpoints
...
8. **Transcript Upload Endpoints**
   ...
   - `GET /api/transcripts/:id` - Get specific transcript with chunks
...
10. **Search & RAG Endpoints**
    - `POST /api/search` - Semantic search with similarity scoring
    - `POST /api/ask` - RAG-based Q&A using GPT-4o-mini
...
4. Test RAG: POST `/api/ask` with question "What did the client discuss?"
5. Verify results contain relevant chunks with similarity scores

**Success Criteria:**
- ✅ All endpoints respond without errors
- ✅ Transcript uploads successfully
- ✅ Embeddings generate correctly
- ✅ Search returns relevant results
- ✅ RAG generates coherent answers with sources
...
12. **Interactive Demo UI (Optional)**
    - Create `public/index.html` with search/ask interface
    - Mode toggle (Search vs Ask)
    - Upload section (text + PDF with date picker)
    - Results display with similarity badges
    - **Note**: This is optional for Phase 1 since Custom GPT is primary interface

13. **Create OpenAPI Schema**
    - Create `api/openapi.json` with API specification
    - Add endpoint: `GET /openapi.json` to serve schema
    - Document all endpoints for Custom GPT:
      - `POST /api/search`
      - `POST /api/ask`
...
15. **Test Custom GPT Integration**
    ...
    **Test Scenario 1: Upload → Search**
    1. Upload a test transcript via API or demo UI
    ...
    **Test Scenario 3: RAG-based Q&A**
    1. Ask Custom GPT: "What progress has the client made on their confidence goals?"
    2. **Verify**: Custom GPT uses `/ask` endpoint
    3. **Verify**: Answer includes source attribution (meeting dates, similarity scores)
...
**Success Criteria:**
- ✅ Custom GPT successfully calls API endpoints
- ✅ Fresh transcripts are searchable immediately after upload
- ✅ RAG-based answers include source attribution
...
- **Chat**: OpenAI `gpt-4o-mini` for RAG
