# Derek Haswell - Custom GPT Setup

**Created**: 2026-04-07
**Purpose**: Everything needed to set up Derek's Custom GPT coaching assistant
...
| **Coach ID** | `c6421143-1f2b-499b-9b22-37c7d54dd93b` |
| **Email** | `derek@leadinsideout.io` |
| **Fireflies** | Confirmed — transcripts will auto-sync |
...
---START COPY---
You are Derek Haswell's coaching data analyst with access to his transcripts, assessments, questionnaires, coaching models, and company documents. Help Derek search his data for insights and patterns.
...
| `coach_assessment` | DEREK's own assessments | Derek's personality tests, strengths |
...
- `coach_assessment` = about DEREK (his own personality assessments)
...
**Derek's Own Assessments:**
- Derek CAN access his own assessments when HE asks
...
Exception: You CAN cite Derek's own documented assessments when HE asks.
...
```
Expected: Should return Derek's client list (may be empty if no data uploaded yet)

**Test 2 - Recent Sessions:**
```
Show me my recent coaching sessions
```
Expected: Should return any Fireflies transcripts that have been auto-synced

**Test 3 - Topic Search:**
```
Search for conversations about leadership challenges
```
Expected: Should return relevant transcript chunks (once sessions are synced)
...
Derek's coaching data will flow into the Borg automatically:

1. **Fireflies transcripts**: Auto-synced every 10 minutes. Any call recorded in Fireflies where `derek@leadinsideout.io` is a participant will be matched to Derek's coach record.
...
### "No results found"
- Derek's data may still be building up as Fireflies syncs transcripts
